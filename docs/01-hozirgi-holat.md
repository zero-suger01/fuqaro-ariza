...
- **Admin API:** complaints (pagination+filtrlar+detail+status+assign+replies+comments), departments/categories/keywords/users CRUD, keyword-suggestions, dashboard. RBAC (B6, 2 rol — `department_staff` bo'lim cheklovi bilan, `admin` hammasi) va rol asosidagi status ruxsatlari ishlaydi. AI ishonchli bo'lganda murojaat avtomatik bo'limga biriktiriladi (B6, `classify_complaint` worker).
