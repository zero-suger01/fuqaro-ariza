...
|---|---|---|---|
| `GET /api/admin/complaints` — **pagination envelope'ga o'tadi** + filtrlar: `priority`, `department_id`, `assigned_user_id`, `source`, `overdue=true`, `needs_review=true`, `q` (ticket/telefon/matn); **v1.4 navbat filtrlari:** `unassigned=true` (bo'limi yoki egasi yo'q, terminal emas), `sla_risk=true` (muddatning ≥75% o'tgan, hali overdue emas), `need_info_over_hours=24` (shu soatdan ko'p `need_info` da turgan), `mine=true` (`assigned_user_id` = so'rovchi xodim), `stuck_ai=true` (`status=new` va 1 soatdan ko'p — LLM ishlamagan); **v1.9:** `stage=new\|progress\|done` (bosqich tabi — 10 ta statusni 3 ga yig'adi, xarita `app/core/constants.py::STAGES`; noma'lum qiymat e'tiborsiz qoldiriladi); har bir qatorda `department` (B6) va `assigned_user` | ro'yxat | department_staff+ | P1 |
| `GET /api/admin/complaints/stage-counts` (v1.9) → `{"new": int, "progress": int, "done": int}` — bosqich tablari uchun sonlar. `GET /complaints` bilan AYNAN bir xil filtrlarni qabul qiladi, `stage`dan tashqari (har bosqich alohida sanaladi); rol chegarasi ham bir xil (`department_staff` faqat o'z bo'limini sanaydi). Sonlarni sahifadagi yozuvlardan hisoblab bo'lmaydi — bir sahifada 20 ta yozuv bor, ya'ni «20/0/0» kabi yolg'on ko'rsatkich chiqardi | ro'yxat tablari | department_staff+ | v1.9 |
