...
- [x] **T2.1 (S)** Status o'zgarganda/javob yuborilganda backend worker `sendMessage` chaqiradi (shablonlar 4 tilda, fuqaro tili bo'yicha). Bot protsessi bunga aralashmaydi.
- [x] **T2.2 (S)** need_info: xabar ichida xodimning savoli + "Javob yozish" tugmasi → fuqaro yozgani `POST /api/bot/complaints/info` orqali backendga ketadi ([03](03-kontraktlar.md) §6, v1.4 da kontraktga qo'shildi; `info_provided` event, murojaat avtomatik `in_progress` ga qaytadi).
- [x] **T2.3 (S)** `resolved` xabariga «Hal bo'ldimi? [Ha] [Yo'q]» inline tugmalari → `POST /api/bot/complaints/feedback` ([03](03-kontraktlar.md) §3.6/§6). «Yo'q» → izoh so'raladi, murojaat qayta ochiladi.
