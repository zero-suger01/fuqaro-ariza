...
Audit sanasi: 2026-07-24, `main` (B1+B2+B3 backend, F1 frontend guest oqimi — tugallangan, AI coder solo sessiya). Bu hujjat — "nima bor, nima yo'q" ning haqiqiy manzarasi. Yangi tasklar shu gap'lardan kelib chiqqan.
...
- **Admin qismi (`/admin/*`, `/login`, `/register`) — ESKI HOLATDA, bu sessiyada tegilmadi.** Staff login yangi backend bilan ishlaydi (sinovdan o'tkazilgan) va dashboard asosiy statistikasi ham to'g'ri ko'rinadi (eski/yangi endpoint shakli mos kelib qoldi), LEKIN murojaatlar ro'yxati sahifasi yangi pagination envelope (`{items,total,...}`) bilan ishlamaydi — eski kod massiv kutadi. **F2 workstream shu qismni to'liq qayta quradi.**
- Saqlanadigan narsalar: dizayn tokenlari (navy+gold), `MapPicker`, `Button`/`Card`/`Badge` UI kiti (`GuestButton` shular naqshiga mos yozildi).
...
**Hali ochiq:** K7 (rate limit/captcha — B4.3/B4.7), K8 (statistika UTC — B5.6), K11 (JWT localStorage), admin mobil drawer (F1.8/F2).

## 3. TZ va yangi talablar bo'yicha YO'Q narsalar (yangilangan gap-jadval)

| Soha | Yo'q narsa | Qayerda hal qilinadi |
|---|---|---|
| Fuqaro UX | **Tayyor** (F1) — wizard, holat, i18n, ovoz. Qolgan: QR landing `/go` (F3.1), fuqaro kabineti UI `/kabinet` (F3.3) | [06](06-frontend-tasklar.md) F3 |
| Admin panel | Yangi kontraktga moslashtirish (pagination, AI paneli, javob editori, keyword boshqaruvi, RBAC UI) | [06](06-frontend-tasklar.md) F2 |
| i18n | Backend + FE tayyor (F1). Admin hali faqat uz (rejalashtirilganidek) | — |
...
Guest oqim (backend+frontend) endi **to'liq ishlaydi va sinovdan o'tgan** — loyihaning yuragi (checkpoint C1 asosiy qismi) tayyor. Qolgan eng yuqori qiymatli yo'nalishlar:
...
2. **F2** — admin panelni yangi kontraktga qayta qurish (murojaatlar ro'yxati hozir butunlay buzilgan — pagination shakli mos emas).
3. **B4.\*** — SMS (Eskiz), rate limit, captcha — pilotdan oldin xavfsizlik uchun muhim.
